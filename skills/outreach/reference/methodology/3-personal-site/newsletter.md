...
### Substack:feed 的 `<webMaster>`

**几乎每个刊物都有**,值要么是转发到作者的平台地址,要么是作者自己的域名/私人邮箱。
一次请求,不解析页面。

**它的 JSON 接口一个邮箱字段都不给** —— 那些接口给的是**身份**:作者真名与 handle
可以直接取,不必从页面猜。判重的键 `(人, 平台)` 因此在这个平台上最扎实。

**Substack 跳过 `/about`** —— 相对 `<webMaster>` 的净增量只有个位数,不值一次请求。
