# TODO

- [x] 首批设计文档:五阶段流水线、身份与去重、取数层、Sheet 契约。
- [x] 确定写哪张表:**目标表**(实体),不是联系记录表(事件)。零列改动。
- [x] 确定范围:止于拿到联系方式;发信、分层、成交跟踪、点击结算全部划出。
- [x] 去重方案定案:`(人, 平台)` 查 log,不解析链接。实测按 URL 去重会误杀真人。
- [x] log 的落地格式定案:用户级目录、一渠道一文件、只追加,见 `docs/design/storage.md`。
- [ ] 首次播种没做 —— 目标表现有 409 行还没进判重库,下一轮会把表里的人当新人重发现一遍。
      `Store.seed` 已实现并有测试,缺的只是一次带 Sheets scope 的读取。
- [ ] 种子:目录型的入口 URL、搜索型的查询词表。词表现在不存在——`Topic Area` 是自由文本,
      大多只出现一两次,不能直接当词表用。
- [ ] 一次性回填:把只在联系记录表里出现、不在目标表的那批人按名字补进目标表,状态标已联系。
      这批无主页链接,只能靠名字兜底(只标记不丢弃)。
- [ ] 目标表现存重复:按主页链接有一批重复行。流水线只保证不新增,历史清理是人的决定。
- [ ] 实现顺序:先视频站公开频道页,再赞助位市场目录——这两条已被现有数据证明走得通且不需登录。
- [ ] **Sheets 写入被 token scope 挡住** —— `gcloud auth print-access-token` 拿到的是 cloud-platform
      scope,Sheets API 返 403 `ACCESS_TOKEN_SCOPE_INSUFFICIENT`,且刷新需要交互式登录。
      合格行已 stage 在 `~/.local/share/outreach/pending-sheet-rows.jsonl`,
      重新授权后跑 `--append-sheet` 即可。**先对副本跑通再碰正式表这条还没做。**
- [x] 早先实验抓的 1176 个 Reddit 资料页入库 —— adapter 多一条来源(已抓产物导入),
      不重抓、不重付。
- [ ] Reddit 还有一条验过但没实现的免费路:**r/Substack 强制发帖人把 newsletter URL 放进
      author flair**,`rdt -o` 存的原始 Listing 里就带 `author_flair_text`。
- [ ] 实验只抓了 2516 个作者里的 1176 个,**剩约 1340 个无外链作者的资料页没抓**。
      抽样测出这批里 18.1% 在资料页挂了链接、12.5% 有自有域名 —— 是已知的、可估算的剩余产出。
- [ ] 补 `docs/testing.md` 与测试映射;需要时再建 `docs/bugs/`、`dev_skills/`、`workflows/`。
- [ ] **前两档还有三个渠道没有 adapter** —— YouTube、X、Threads。前者免 key
      (频道 about 页在初始 HTML 里),卡在发现那一步没实现;后两个要付费/认证取数层。
      「每档每渠道 10 条」在补齐之前只覆盖得到 1-social 的四个。

## skill(`skills/outreach/`)

- [ ] 本地接上:仓库根的 `skills/` 不被 Claude Code 自动发现,要做软链或 marketplace 配置。
- [ ] 数据层逐个探索并回填 `reference/datalayer/`。已确定边界:Instagram、TikTok
      (均走 ScrapeCreators)、Reddit(`rdt` CLI)、LinkedIn、X。
      **免认证那一档只写了结论,各渠道的具体入口未跑通。**
- [ ] `reference/methodology/` 里除已实测的几份外仍是骨架,正文混着未验证的经验断言
      (页面结构、字段名、路径名)。用之前先核实。
- [ ] 建 `reference/scripts/` 收数据层的能力探针,计划见
      [2026-08-07-reference-scripts.md](../plans/2026-08-07-reference-scripts.md)。
      两处待裁决:数据层正文能否链探针(倾向不能);渠道探针等并行的 `channels/` 重写落地再补。
      现有 `data/experiments/` 里的 reddit 脚本已被 `src/` 覆盖,不迁移。

## 渠道分类收窄的余项

计划见 [plans/2026-08-06-channel-taxonomy.md](plans/2026-08-06-channel-taxonomy.md)。

- [ ] 目标表现有 62 行 Website 有多少符合收窄后的「分发站」定义。历史行不清洗,但要知道比例;
      比例太低说明这一档的历史产出大部分是产品站。
- [x] `media.md` 的域名清单移进 `config/` 的共享域名清单,那份 md 只留判据。
- [ ] **DEV.to 的卖买比例没测** —— 这一档最关键的缺口。已抓到的标题与 tag 分布够判,
      不需要新请求。测出来才知道大 blog 平台排第二这个优先级站不站得住。
- [ ] DEV.to 剔掉 GitHub 那一跳后,只走 `website_url` 的最终邮箱命中率。现有可触达数字
      含 GitHub,不能直接用。
- [ ] Hashnode 的赞助通道有没有**公开可翻的创作者目录**。有的话它比标签页强 ——
      列在那里的人本身就是卖方,省掉卖买判别那一步。
- [x] 自有域名 12/12 换更大样本会掉多少 —— 215 个作者实测掉到 24%,已回填文档。
- [ ] freeCodeCamp News 的五百多个作者里有多少还活着(sitemap 收的是历史全量,且没有 `lastmod`)。
- [ ] HackerNoon 与 Bear Blog 都卡在**资料页/发现页对诚实 UA 返回质询页**。不绕。
      若将来接入带登录态的取数层,这两个可以重新评估。
- [ ] **WordPress.com 只有时间序**,样本里混着大量垃圾账号。找不找得到带互动或时间窗的排序,
      决定这个入口能不能真用 —— DEV.to 上这一条是决定性的。
- [ ] WordPress.com → Gravatar 那两成邮箱只在一个标签的三十个名字上测过,换垂类会不会掉。
- [ ] Micro.blog 的翻页与话题取法未探,只取过首屏 50 条。
- [x] freeCodeCamp News 与 HackerNoon 归哪一档 —— 新开 `5-media/`,有编辑审的多作者刊物
      单成一档,排在最后。目标仍是作者,不是刊物。
- [ ] Tumblr 的匿名 bearer token 那条路**已否决**(绕 key 闸)。若将来接入带登录态的取数层,
      走正规 API key 重评,别再走那条。
- [ ] Paragraph 的受众指标(免 key 的订阅数)是这一档唯一直接可读的。**平台本身按垂类否决了**,
      但这个字段的形状值得在别处复用:找有没有别的平台也免 key 给订阅数。
- [ ] `cost-ranking.md` 只有一个轴(抓取 `credit/人`),缺**合作成本/人**那一轴。
      数据只能从实际谈过的人身上来,属发信侧产出。在有数之前用代理信号:
      有没有 rate card / 招商页。
- [ ] 媒体的作者页当发现源值不值得跑:作者页是高纯度人名列表,但要三跳且中途断掉过半,
      而产出的人还得再去别的平台找他的主场。**未承诺**,现在按不做处理。
- [ ] 公司站挖个人的做法(`/team` 三跳、名单页分流、按站分流的命中率)随目标收窄一起退役,
      原文在 `website.md` 的 git 历史里。若将来要做同行/合作方名单,从那里取回。

## 前两档实测暴露的事(2026-08-07)

- [ ] **规模闸门对前两档基本不生效。** 九个渠道里只有三个给粉丝数
      (Mastodon / Instagram / TikTok),其余六个拿不到,闸门只能退到「自有站上有没有招商证据」。
      **这是「每渠道 10 条」凑不够的唯一原因** —— 候选有几百个,卡的是判据不是发现。
      要么给这些渠道找一个真的受众代理,要么承认这条带只适用于给粉丝数的渠道。
- [ ] **Mastodon 的账号目录出不了规模** —— 196 个人里只有三个粉丝数过五千。
      邮箱命中三成七是各档最高,但规模不够。要换入口(话题时间线、更大的实例)再测一轮。
- [ ] freeCodeCamp News 与 Hashnode 的自有域名比例与最终邮箱命中率,两个 adapter 刚补上,
      数字待第一轮跑完回填。
- [x] 邮箱误报三类已修并有测试:平台自己的信箱(`partners@dev.to`、`git@github.com`)、
      图片文件名(`x.jpeg@1f.png`)、职能地址扩充。
- [x] `redd.it` 当自有域名 —— 已修,污染的 110 条判重记录已清。
- [x] CSS 类名被当正文读(Astra 主题的 `ast-masthead-*` 让个人博客被判成多作者刊物)—— 已修。
- [x] 共享域名按子串匹配 —— `t.co` 吃掉 `troyhunt.com`。改成按 label 匹配。

## 首批实测暴露的事(2026-08-06)

- [ ] **Substack feed 里已经没有 `<webMaster>`** —— `newsletter.md` 记的那条取邮箱的路失效,
      实测 10 个刊物零命中。Substack 侧目前没有可替代的公开邮箱字段,只剩自定义域名走第二跳。
- [ ] **Instagram adapter 头一轮解析全错**(SociaVault 的响应多包了一层信封),8 次调用白花。
      信封已修、`edge_followed_by.count` 已接上、fixture 测试已钉住,**但修完之后一次都没真跑过**。
      修完之后**用落盘的 raw 回放了那 8 条**,不花 credit —— 解析确认无误(粉丝带生效),
      但那批账号本身粉丝数只有几十,`instagram` 的搜索词选得太窄。
- [ ] **Instagram 的发现质量**:`instagram/search` 返回的是名字里带关键词的账号,不是有受众的账号。
      要么换词、要么改走 hashtag 那条端点。这是它 0 产出的真正原因,不是解析。
- [ ] SociaVault 余额只剩 2 credits。充值后重跑 Instagram 与 TikTok。
- [ ] **邮箱抽取有两类误报,已修但只在这一批上验过**:fediverse handle
      (`@user@instance` 正好匹配邮箱正则)与职能地址(`support@` / `subscriptions@` 那类)。
- [ ] **多作者刊物判据不够** —— 只靠根页上的作者链接数,漏掉了 thenextweb、atlasobscura 这类。
      现在靠域名清单兜,清单不可穷尽。
- [ ] **6/8 的渠道拿不到粉丝数**,规模带对它们不生效,闸门只能退到「站上有没有招商证据」。
      这条退路的产出率极低(podcast 7 个有邮箱的全部落在「规模未核实」)。需要给这些渠道
      找一个真的受众代理指标,否则「5000–100000」这个门槛对多数渠道形同虚设。
- [ ] **WordPress.com 样本质量确认很差** —— 10 个作者里 9 个拿不到联系方式,且明显混着垃圾账号,
      与 methodology 里已记的「只有时间序」一致。

## X 的现状(2026-08-07 复验)

- [ ] **只用 ScrapeCreators 的前提下,X 没有发现能力。** 它的 Twitter 只有
      `profile` / `user-tweets` / `tweet` / `community`,**没有任何用户或内容搜索端点**
      (`search`、`search/users`、`users/search`、`tweets/search` 一律 404)。
      给已知 handle 能富化,但产不出新名字 —— 与 LinkedIn 同一个裁决形状。
- [ ] `methodology/1-social/twitter-x.md` 里「话题搜索能找到人」只在**走用户本人账号**
      那条路上成立,不适用于当前供应商。正文没有标出这个前提。
- [ ] 两条尚未评估的发现路子:X Community 的成员发帖(要先有 community id),
      以及从 DEV.to 的 `twitter_username` 反查(那是富化,不产生新人)。

## 第五档媒体的余项

- [ ] **受雇记者占比没测** —— 决定这一档整体值不值得。受雇记者的受众是刊物的,
      离职带不走,不是目标;自由撰稿人才是。判据是他有没有自己的主场。
- [ ] `5-media/index.md` 里那批未探的刊物(Towards Data Science、The New Stack、
      InfoQ、VentureBeat 等)一个都没跑过。
- [ ] HackerNoon 的资料页质询页,接入带登录态的取数层后重评。

## 将来的选项

- **与 auto-gtm 合并**(已定方向)。它的取数层已解决认证:复用浏览器登录态、单一入口、分档降级、
  只读白名单。合并后本项目发现阶段改为经它取数,不自建认证。现在不引依赖——它覆盖 X / Reddit,
  与本项目已验证的两条路不重叠。设计已按「取数方式可替换」写好,换的时候其余四阶段不动。
- **认证墙后的平台**(X / LinkedIn / Reddit)。现有数据里合计不到一成,且其中多数链接根本不指向
  那些平台。等合并后随取数层一起获得,不单独投入。

## 待确认

- Sheet 共享设置是「任何人有链接即可编辑」,任何拿到链接的人都能改。表属另一位同事,
  需与其确认是否收紧。
- 目标表的平台列现在混着两种含义(人所在平台 / 打算用的联系渠道)。新行按前者写,历史行是否
  清洗待定。

## 参考件的处置

`build-partnership-tracker`(外部 skill)管的是谈成之后的点击跟踪与结算,从「已确认合作」起步,
装不下候选到联系方式这一段,不引入。它的两个缺陷若要复用其代码需先修:归档内夹带真实联系人
邮箱;硬编码 origin 只被替换了一半,会静默指向他人账号下的域名。
